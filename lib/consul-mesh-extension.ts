...
let environment: {
  //environment variable is used to hold the environment details from app container
  [key: string]: string;
} = {};
...
export class ECSConsulMeshExtension extends ServiceExtension {
  ...
  public addHooks() {
    ...
    const parentServiceConsulMesh = this.parentService.serviceDescription.get(
      'consul'
    ) as unknown as ECSConsulMeshExtension;

    container.addContainerMutatingHook(
      new ConsulMeshsMutatingHook({ healthCheck: parentServiceConsulMesh.healthCheck })
    );
  }
  ...
  public useTaskDefinition(taskDefinition: ecs.TaskDefinition) {
    const serviceContainer = this.parentService.serviceDescription.get(
      'service-container'
    ) as Container;
    ...
    this.healthSyncContainerName = this.consulChecks
      ? ''
      : this.healthCheck
      ? serviceContainer.container?.containerName
      : '';
    ...
    if (this.healthSyncContainerName) {
      const consulECSHealthSync = taskDefinition.addContainer('consul-ecs-health-sync', {
        ...
        essential: false,
        secrets: this.aclSecretArn
          ? {
              CONSUL_HTTP_TOKEN: ecs.Secret.fromSecretsManager(
                secretsmanager.Secret.fromSecretAttributes(
                  this.scope,
                  'ImportedACLSecret' + this.parentService.id,
                  {
  ...
  private get buildConsulClientCommand(): string[] {
    ...
    return [
      `cp /bin/consul /bin/consul-inject/consul &&
				ECS_IPV4=$(curl -s $ECS_CONTAINER_METADATA_URI | jq -r '.Networks[0].IPv4Addresses[0]') && if [ ${
          this.tls
        } == true ]; then \
  ...
  public useService(service: ecs.Ec2Service | ecs.FargateService) {
    this.consulServerSecurityGroup.connections.allowFrom(
      service.connections.securityGroups[0],
      Port.tcp(8301),
      'allow consul server to accept traffic from consul client on TCP port 8301'
    );
    this.consulServerSecurityGroup.connections.allowFrom(
      service.connections.securityGroups[0],
      Port.udp(8301),
      'allow consul server to accept traffic from consul client on UDP port 8301'
    );
    this.consulServerSecurityGroup.connections.allowFrom(
      service.connections.securityGroups[0],
      Port.tcp(8300),
      'allow consul server to accept traffic from the service client on TCP port 8300'
    );
    ...
    const serviceSecurityGroupIds = service.connections.securityGroups.map(
    ...
    const cfnParentService = this.parentService.ecsService.node.findChild(
      'Service'
    ) as ecs.CfnService;
  ...
  public connectToService(otherService: Service, connectToProps: ConnectToProps = {}) {
    ...
    this.upstreamStringArray.push(
      upstreamName + ':' + (connectToProps.localBindPort ?? this.upstreamPort)
    );

    var cfnTaskDefinition = this.parentService?.ecsService?.taskDefinition?.node
    ...
    cfnTaskDefinition.addPropertyOverride('ContainerDefinitions.2.Command', [
      'mesh-init',
      '-envoy-bootstrap-dir=/consul/data',
      '-port=' + serviceContainer.trafficPort,
      '-upstreams=' + this.buildUpstreamString,
    ...
    if (this.parentServiceEnvironments.length == 0) {
      // add environment variables from app container only once
      for (const [key, val] of Object.entries(environment)) {
        this.parentServiceEnvironments.push({ Name: key, Value: val });
      }
    }

    this.parentServiceEnvironments.push({
      Name: upstreamName.toUpperCase() + '_URL',
      Value: 'http://localhost:' + (connectToProps.localBindPort ?? this.upstreamPort++),
    });

    //Also add required environment variables
    cfnTaskDefinition.addPropertyOverride(
      'ContainerDefinitions.0.Environment',
      Array.from(this.parentServiceEnvironments.values())
    );
  }
...
export class ConsulMeshsMutatingHook extends ContainerMutatingHook {
  ...
  public mutateContainerDefinition(
